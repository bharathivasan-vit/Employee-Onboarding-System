...
public interface UserDetailsRepo extends JpaRepository<UserDetailsEntity, Integer> {
    ...
    //    Admin
    @Query(value = "SELECT user_name AS userName,phone_number AS phoneNumber,email_id AS emailId,role_id AS roleId,otp_verified AS otpVerified from ipr.eob_user_details where otp_verified = true ORDER BY user_id ASC ",nativeQuery = true)

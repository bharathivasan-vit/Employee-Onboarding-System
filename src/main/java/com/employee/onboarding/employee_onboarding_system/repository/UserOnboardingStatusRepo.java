...
public interface UserOnboardingStatusRepo extends JpaRepository<UserOnboardingStatusEntity, Integer> {
    ...
    @Query(value = "SELECT COUNT(*) FROM ipr.eob_user_onboarding_status WHERE user_id = ?1 AND check_list_id = ?2", nativeQuery = true)
    int existingUserAndChecklist(int userId,int checkListId);
}